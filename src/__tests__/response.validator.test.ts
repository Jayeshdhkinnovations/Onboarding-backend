...
describe("updateResponseStatusSchema - Unit Tests", () => {
  ...
  it("should reject invalid status values like 'draft', 'archived', 'invalid', 'pending', 'partial'", () => {
    ...
    const invalid4 = updateResponseStatusSchema.safeParse({ status: "pending" });
    expect(invalid4.success).toBe(false);

    const invalid5 = updateResponseStatusSchema.safeParse({ status: "partial" });
